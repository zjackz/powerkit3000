    ...
    public class SplitCommand
    {
        ...
        public async Task ExecuteAsync(string filePath, int numberOfFiles)
        {
            ...
            try
            {
                ...
                int recordsPerFile = (int)Math.Ceiling((double)totalRecords / numberOfFiles);
                if (recordsPerFile == 0) recordsPerFile = 1; // Ensure at least one record per file if totalRecords > 0
                ...
                using (JsonTextReader reader = new JsonTextReader(file))
                {
                    ...
                    while (reader.Read())
                    {
                        if (reader.TokenType == JsonToken.StartObject)
                        {
                            ...
                            {
                                string outputFileName = Path.Combine(Path.GetDirectoryName(filePath)!, $"{Path.GetFileNameWithoutExtension(filePath)}_part{currentFileIndex + 1}.json");
                ...
                else if (currentFileIndex == 0 && totalRecords > 0) // Case where only one file is created and currentBatch is empty (e.g., numberOfFiles=1, totalRecords=5)
                {
                     Console.WriteLine($"已创建文件: {Path.Combine(Path.GetDirectoryName(filePath)!, $"{Path.GetFileNameWithoutExtension(filePath)}_part1.json")}，包含 {totalRecords} 条记录。进度: 100%");
                }
