...
- **Next 代 Control Tower（`frontend/tradeforge-suite-next`）**：新增 Next.js App Router + Ant Design Pro 架构，引入混合渲染、ProLayout 导航体系、ProCard/StatisticCard 指标组件与主题 token 统一管控。仪表盘页面重构为“TradeForge Control Tower”，内嵌 `SystemHealthPanel` 复用同一套聚合逻辑展示 Kickstarter 导入与 Amazon 采集指标，默认每 30 秒刷新一次；并包含 KPI 顶栏、指标卡片、趋势图、爆款榜单与类目占比图示例，后续会逐步接入真实 API 与实时数据。
