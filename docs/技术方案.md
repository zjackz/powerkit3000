...
backend/pk.consoleapp ──► AppDb (Postgres)
            │                                 │
            ▼                                 ▼
 backend/pk.core ───────► backend/pk.api ──► frontend/tradeforge-suite-next
        (Kickstarter + Amazon 领域服务)         (REST + Amazon 模块)        (MISSION X 前端)
```
...
### 2.5 前端
- **MISSION X（`frontend/tradeforge-suite-next`）**：基于 Next.js App Router + Ant Design Pro，整合 Kickstarter 分析、亚马逊榜单与亚马逊运营三大模块；提供 Dashboard KPI 汇总、系统健康监控、项目巡检表格、收藏驾驶舱、Amazon 运营中控、趋势雷达与抓取调度页面；组件层复用 ProCard/StatisticCard，React Query 与自定义服务共享回退逻辑，页面默认 30 秒刷新关键指标。
